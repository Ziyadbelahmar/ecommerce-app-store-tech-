...
function AdminDashboard() {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case "delivered":
        return { label: "Delivered", color: "#00ff00", icon: <FiCheckCircle /> };
      case "shipping":
        return { label: "Shipping", color: "#5ac8fa", icon: <FiTruck /> };
      case "processing":
        return { label: "Processing", color: "#ffcc00", icon: <FiClock /> };
      case "confirmed":
        return { label: "Confirmed", color: "#5ac8fa", icon: <FiCheckCircle /> };
      case "cancelled":
        return { label: "Cancelled", color: "#ff3b30", icon: <FiX /> };
      default:
        return { label: "Unknown", color: "#666", icon: <FiPackage /> };
    }
  };
  ...
  const getAvatarColor = (name) => {
    const colors = [
      '#c4ff0d', '#ff6b35', '#4a90e2', '#5ac8fa', 
      '#af52de', '#ffcc00', '#30d158', '#ff2d55',
      '#ff9500', '#00c7be'
    ];
          ...
          <div style={styles.headerLeft}>
            <div style={styles.logoContainer}>
              ...
              <div>
                ...
                <p style={styles.subtitlePro}>Welcome back, <span style={{color: '#c4ff0d'}}>{user?.name || "Admin"}</span></p>
              </div>
            </div>
          </div>
            ...
            <div style={styles.insightsGrid}>
              <div style={styles.insightCard}>
                ...
                <p style={styles.insightText}>
                  {orders.length} orders processed today
                </p>
              </div>
              <div style={styles.insightCard}>
          ...
          <div style={styles.content}>
            ...
            <div style={styles.sectionHeader}>
              ...
              <div style={styles.headerActions}>
                ...
                <button onClick={handleExportProducts} style={styles.exportBtnPro}>
                  <FiDownload style={{ marginRight: "8px" }} />
                  Export
                </button>
                <button onClick={handleAddProduct} style={styles.addBtnPro}>
                        ...
                        <td style={styles.tableCellPro}>
                          <span style={{
                            ...
                            ...(product.stock === 0 ? styles.stockBadgeOut :
                                product.stock < 10 ? styles.stockBadgeLow :
                                styles.stockBadgeIn)
                          }}>
                            {product.stock} units
                          </span>
              ...
              {orders.map((order) => {
                ...
                return (
                  <div key={order._id} style={styles.orderRowPro}>
                    ...
                    <div style={{...styles.orderStatusPro, color: statusInfo.color, borderColor: statusInfo.color}}>
                      {statusInfo.icon}
                    ...
                    <div style={styles.orderActionsPro}>
                      ...
                      <button onClick={() => handleDeleteOrder(order._id)} style={styles.deleteActionBtnPro}>
                        <FiTrash2 />
                      </button>
                    </div>
                  </div>
          ...
          <div style={styles.content}>
            <div style={styles.sectionHeader}>
              <div>
                ...
                <p style={styles.sectionSubtitlePro}>
                  {users.length} total customers • {users.filter(u => !u.isAdmin).length} active
                </p>
              </div>
              <button onClick={() => {
                const csvData = [
                ...
                {productForm.images.map((image, index) => (
                  <div key={index} style={styles.imageInputRow}>
                    ...
                    {productForm.images.length > 1 && (
                      <button onClick={() => removeImageField(index)} style={styles.removeImageBtnPro}>
                        <FiX />
                      </button>
                    )}
                  </div>
...
const styles = {
  ...
  adminBadgePro: {
    ...
    alignItems: "center",
    padding: "12px 20px",
    background: "linear-gradient(135deg, #c4ff0d 0%, #9acc0a 100%)",
    color: "#000",
    fontSize: "13px",
    fontWeight: "800",
    borderRadius: "12px",
  ...
  tabActivePro: {
    ...
    color: "#c4ff0d",
    boxShadow: "0 0 20px rgba(196, 255, 13, 0.2)"
  },
  tabBadgePro: {
    padding: "4px 10px",
  ...
  statCardPro: {
    ...
    ":hover": {
      ...
      boxShadow: "0 8px 30px rgba(196, 255, 13, 0.1)"
    }
  },
  statCardHeader: {
    display: "flex",
  ...
  // CUSTOMERS ULTRA PRO
  customersGridUltraPro: {
    display: "grid",
    gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))",
    gap: "24px"
  },
  customerCardUltraPro: {
    padding: "28px",
    backgroundColor: "#0f0f0f",
    border: "1px solid #1a1a1a",
    borderRadius: "20px",
  ...
  customerCardHeaderPro: {
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: "20px"
  },
  customerAvatarUltraPro: {
    width: "72px",
    height: "72px",
    borderRadius: "18px",
    display: "flex",
